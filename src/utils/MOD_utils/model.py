...
def load_coco_pretrained_model(opt, model):
    if opt.arch == 'dla_34':
        ...
        model_path = '../coco_resdcn18.pth'
    elif opt.arch == 'resnet_101':
        print('load coco pretrained resnet_101')
        model_path = '../coco_resdcn101.pth'
    else:
        raise NotImplementedError
    
    checkpoint1 = torch.load('../coco_resdcn101.pth', map_location=lambda storage, loc: storage)
    checkpoint2 = torch.load('../coco_resdcn18.pth', map_location=lambda storage, loc: storage)
    print('loaded {}, epoch {}'.format('../coco_resdcn101.pth', checkpoint1['epoch']))
    print('loaded {}, epoch {}'.format('../coco_resdcn18.pth', checkpoint2['epoch']))
